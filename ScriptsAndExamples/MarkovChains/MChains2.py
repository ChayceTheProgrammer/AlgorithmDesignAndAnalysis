...
def simulate_markov_chain(transition_matrix, cities, steps, start_city):
    ...
    path = [start_city]
    print(f"The hero starts in {start_city}.")
    ...
    for step in range(steps):
        ...
        print(f"Step {step + 1}: Moving from {cities[current_city]} to {cities[next_city]} (Probability: {transition_matrix[current_city][next_city]:.2f})")
...
print(f"The hero's complete journey: {' -> '.join(journey)}")